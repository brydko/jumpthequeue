...
public interface QueuemanagementRestService {
  ...
  /**
   * Delegates to {@link Queuemanagement#findQueueCto}.
   *
   * @param id the ID of the {@link QueueCto}
   * @return the {@link QueueCto}
   */
  @GET
  @Path("/queue/cto/{id}/")
  public QueueCto getQueueCto(@PathParam("id") long id);

  /**
   * Delegates to {@link Queuemanagement#findQueueCtos}.
   *
   * @param searchCriteriaTo the pagination and search criteria to be used for finding queues.
   * @return the {@link Page list} of matching {@link QueueCto}s.
   */
  @Path("/queue/cto/search")
  @POST
  public Page<QueueCto> findQueueCtos(QueueSearchCriteriaTo searchCriteriaTo);
